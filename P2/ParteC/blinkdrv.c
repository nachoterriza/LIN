...
#define NR_SAMPLE_COLORS 9 

//unsigned int sample_colors[]={0x111111, 0x110000, 0x110100, 0x111100, 0x001100, 0x000011, 0x010010, 0x100011, 0x000000};
			  ...
			  size_t len, loff_t *off)
{
	...
	char messages[NR_LEDS][NR_BYTES_BLINK_MSG];
	char kbuf[BUFF_SIZE];
	...
	if(copy_from_user(kbuf,user_buffer,bytes_to_read))
		return -EFAULT;

	printk(KERN_INFO "hemos leido :%c%c%c%c%c%c%c%c%c%c%c%c%c%c\n",kbuf[0],kbuf[1],kbuf[2],kbuf[3],kbuf[4],kbuf[5],kbuf[6],kbuf[7],kbuf[8],kbuf[9],kbuf[10],kbuf[11],kbuf[12],kbuf[13]);
	
	kbuf[bytes_to_read] = '\0';

		/* zero fill*/
		memset(messages,0,NR_BYTES_BLINK_MSG*NR_LEDS);
	while(kbuf_ptr != NULL)
	{
		tokens = strsep(&kbuf_ptr, ",");

	printk(KERN_INFO "hemos leido :%c%c%c%c%c%c%c%c%c%c%c\n",tokens[0],tokens[1],tokens[2],tokens[3],tokens[4],tokens[5],tokens[6],tokens[7],tokens[8],tokens[9],tokens[10]);
		if(sscanf(tokens, "%d:%x", &ledn, &color)==2) {
			if(ledn <= 7 && ledn >= 0) {
				printk(KERN_INFO "ledn: %d, color: %x", ledn, color);
				/* Fill up the message accordingly */
				messages[ledn][3]=((color>>16) & 0xff);
			 	messages[ledn][4]=((color>>8) & 0xff);
			 	messages[ledn][5]=(color & 0xff);
				printk(KERN_INFO "Leida info sobre led %i\n", ledn);
			
			}
		} 
	printk(KERN_INFO "una vuelta mas\n");
	}
	
	for(;i<NR_LEDS;i++) {
		messages[i][0]='\x05';
		messages[i][1]=0x00;
		messages[i][2]=i; /* Change Led number in message */

		printk(KERN_INFO "impreso %i\n",i);
