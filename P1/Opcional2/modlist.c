...
void add_item_list_first(struct list_head *list, int data)
{
	list_item_t *item = vmalloc(sizeof(list_item_t));
	item->data = data;
...
{
	list_item_t *pos = NULL;
	list_sort(pos,list,<);
}
*/
	

void remove_item_first(struct list_head *list)
{
list_item_t *pos, *n = NULL;
int t = 0;
	list_for_each_entry_safe(pos,n,list,links)
...
static ssize_t modlist_write(struct file *filp, const char __user *buf, size_t len, loff_t *off)
{
	...
	*off+=len;            /* Update the file pointer */
    ...
    else if(sscanf(buff_kern,"cleanup")==1)
	cleanup_list(&my_list);
    else if(sscanf(buff_kern,"push")==1)
	add_item_list_first(&my_list,data);
    else if(sscanf(buff_kern,"pop")==1)
	remove_item_first(&my_list);
    else
    {
    	printk(KERN_INFO "Modlist: invalid argument!!\n");
    	return -EINVAL;
    }
    /*scanned = sscanf(buff_kern,"%s %i",command,&data);

    if(scanned != 2 && scanned != 1)
    {
    	printk(KERN_INFO "Modlist: invalid argument!!\n");
    	return -EINVAL;
    }
